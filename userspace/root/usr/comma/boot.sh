...
create_tmpfiles() {
  mkdir -p /var/crash /var/tmp /var/lib/dpkg /var/lib/logrotate \
    /var/spool/cron/atjobs /var/cache/pollinate
  chmod 1777 /var/tmp 2>/dev/null || true
  touch /var/lib/dpkg/lock-frontend /var/lib/dpkg/status 2>/dev/null || true
}
...
mount_early_state() {
  ...
  chown root:syslog /var/log 2>/dev/null || true
  chmod 775 /var/log 2>/dev/null || true
  create_tmpfiles
...
qcom_early() {
  ...
  boost_early
  [ -e /sys/kernel/boot_wlan/boot_wlan ] && echo 1 >/sys/kernel/boot_wlan/boot_wlan 2>/dev/null || true
...
start_serial_shell() {
  [ -c /dev/ttyMSM0 ] || return 0
  setsid /bin/bash -l </dev/ttyMSM0 >/dev/ttyMSM0 2>&1 &
}
...
/usr/comma/debug.py >/tmp/agnos-debug.log 2>&1 || true
/usr/comma/screen_calibration.py >/tmp/screen-calibration.log 2>&1 || true
start_sound
